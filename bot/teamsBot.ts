...
export class TeamsBot extends TeamsActivityHandler {
  constructor() {
    ...
    this.onMessage(async (context, next) => {
      ...
      const message = TurnContext.removeRecipientMention(context.activity).trim();

      console.log(`messasge: ${message}`);
      const commands = [helpCommand, setApiKeyCommand, deleteApiKeyCommand, forgetConversationCommand, askAiCommand];

      for(let command of commands) {
        if (command.checkCommand(message, context)) {
          await context.sendActivity({ type: 'typing' });

          await command.processCommand(context, message, fromId, conversationId);
          break;
        }
      }
